"""Collect calculation dependencies for a Channel metric plan."""
...
class ChannelRequirementCollector:
    def collect(self, metrics: tuple[Any, ...], catalog: Any) -> ChannelRequirements:
        if not metrics:
            raise ValueError("channel_metric_requires_metric")
        recipe_ids = {metric.recipe for metric in metrics}
        if len(recipe_ids) != 1:
            raise ValueError("channel_metric_recipe_mismatch")
        ...
        for metric in metrics:
            ...
            if calculation is None:
                raise ValueError(f"channel_metric_has_no_calculation:{metric.id}")
            calculations.append(calculation)
        strategies = {item.execution_strategy for item in calculations}
        if len(strategies) != 1:
            raise ValueError("channel_metric_execution_strategy_mismatch")
...
__all__ = ("ChannelRequirementCollector",)
