...
const NumberCard = ({ title, description, value, icon = false, background, height = '', widthIcon = '', suffix = '', refetch = undefined }) => {
  ...
  return (
    <div className={`p-6 max-lg:p-5 max-md:p-4 bg-background-light dark:bg-background-chart-dark dark:border-background-white-15 transition-all duration-300 border border-border-black-10 rounded-2xl shadow-component`} style={{ height: `${height}px` }}>
      <NameChart nameChart={title} description={description} icon={icon} width={widthIcon} backgound={background} getChartData={getEChartsData} refetch={refetch} />
