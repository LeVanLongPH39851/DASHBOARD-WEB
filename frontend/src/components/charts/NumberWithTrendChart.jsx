...
const NumberWithTrendChart = ({
  ...
  // console.log("NumberWithTrendChart");
  ...
  if (data === "isLoading") {
    return (
      <div
        className={`p-6 max-lg:p-5 max-md:p-4 bg-background-light dark:bg-background-chart-dark dark:border-background-white-15 transition-all duration-300 border border-border-black-10 rounded-2xl shadow-component`}
  ...
  const chartOption = useMemo(() => {
    ...
    const lineColor =
      trendPercent > 0 ? "rgba(47, 196, 198, 1)" : "rgba(255, 56, 60, 1)";
    ...
    return {
      ...
      tooltip: {
        ...
        borderWidth: 0,
        textStyle: {
          fontSize: !screenMd ? fontSize.tooltip : "10.5px",
        ...
        formatter: (params) => {
          ...
          const percent =
            totalX > 0 ? ((p.value / totalX) * 100).toFixed(1) : 0;
          return `
            <div style="padding: 4px 8px;">
              <div style="font-weight: 500; font-size: ${!screenMd ? "13" : "11"}px; color: rgba(0, 0, 0, 0.7); margin-bottom: 2px;">
                ${p.axisValue}
              </div>
              <span style="font-size: ${!screenMd ? "12" : "10.5"}px; font-weight: 400; color: rgba(0, 0, 0, 0.7);">
                ${p.value.toLocaleString(undefined, { maximumFractionDigits: nameChart.includes("%") ? 2 : 0 })} ${suffix} <span style="font-size: ${!screenMd ? "11" : "10"}px;">(${percent}%)</span>
              </span>
            </div>
            `;
        },
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center mb-2">
        <span className="text-color-black-100 dark:text-color-white-50 transition-all duration-300 font-normal text-sm max-lg:text-[13px] max-md:text-xs">
          Ngày {currentDate}
        </span>
      </div>
      <div className="mb-6 max-lg:mb-5 max-md:mb-4 flex items-center gap-3 max-lg:gap-2.5 max-md:gap-2">
        <h4 className="text-5xl max-lg:text-4xl max-md:text-4xl text-nowrap text-color-black-100 dark:text-color-white-90 transition-all duration-300 font-semibold">
        ...
        {hasTrend && (
          <div className="flex gap-2 items-center">
            <div
              className={`px-2 max-lg:px-1.5 py-1 rounded-lg flex items-center gap-1 border ${trendPercent > 0 ? "bg-background-succes-type-1 text-color-succes-type-1 border-border-succes-type-1 dark:bg-background-succes-type-1-dark dark:text-color-succes-type-1-dark dark:border-border-succes-type-1-dark" : "bg-background-error text-color-error border-border-error"}`}
            >
              <span className="text-sm max-lg:text-[13px] max-md:text-xs font-semibold">
                {trendPercent.toFixed(1)}
                {unit}
              </span>
              <figure>
                <img
                  src={trendPercent > 0 ? iconArrowUp45 : iconArrowDown45}
                  alt="Icon Arrow Up 45"
                  className="w-2.25 max-lg:w-2 h-2.25 max-lg:h-2"
                />
              </figure>
            </div>
            <span className="text-sm max-lg:text-[13px] max-md:text-xs text-color-black-50 dark:text-color-white-50 transition-all duration-300 font-normal">
              {trendPeriod}
            </span>
          </div>
        )}
      ...
      {hasTrend ? (
        <div className="w-full">
          <ReactECharts
            ...
            style={{
              width: "100%",
              height: !screenMd ? (!screenLg ? 170 : 150) : 115,
            }}
            opts={{ renderer: "canvas", locale: "VN" }}
          />
        </div>
      ) : (
        <div className="h-42.5 max-lg:h-37.5 max-md:h-28.75 w-full"></div>
      )}
    </div>
  );
};

export default React.memo(NumberWithTrendChart);
