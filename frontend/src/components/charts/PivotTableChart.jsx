    ...
    <div className={`${displayName ? 'p-6 max-md:p-4 bg-background-light dark:bg-background-chart-dark dark:border-transparent transition-all duration-300 border border-border-black-10 rounded-2xl shadow-component' : ''}`} style={{ fontFamily }}>
      ...
      <div className="flex justify-between items-center mb-3 max-md:mb-1">
            ...
            {showColumnMenu && (
              <div className="absolute left-0 top-full mt-2 max-md:mt-1 bg-background-light dark:bg-background-dark dark:border-background-white-15 transition-all duration-300 border border-border-black-10 rounded-lg z-50 min-w-55 max-md:min-w-50">
                <div className="p-3 max-md:p-2 max-h-80 max-md:max-h-70 overflow-y-auto">
                  <div className="text-xs max-md:text-[10.5px] font-semibold text-color-black-50 dark:text-color-white-80 uppercase mb-2 px-2 max-md:px-1">
                    Danh sách cột
                  </div>
                  {table.getAllLeafColumns().map((column) => (
                    <label key={column.id} className="flex items-center gap-2 px-3 max-md:px-2 py-2 max-md:py-1.25 cursor-pointer rounded-md text-xs max-md:text-[10.5px] font-medium">
                      <input
                        type="checkbox"
                        checked={column.getIsVisible()}
                        onChange={column.getToggleVisibilityHandler()}
                        className="cursor-pointer w-3 h-3 max-md:w-2.5 max-md:h-2.5 rounded focus:ring-2"
                      />
                      <span className="text-color-black-70 dark:text-color-white-80">{column.id}</span>
                    </label>
                  ))}
                </div>
              </div>
          ...
          {showPagination && (
            <div className='relative cursor-pointer'>
              ...
              <figure className='absolute top-1/2 -translate-y-1/2 left-3 rotate-90'>
                <img src={!stateGlobals.darkMode ? iconShowList : iconShowListDark} alt="Icon Show List" className='w-3 max-md:w-2.5' />
              </figure>
              <figure className='absolute top-1/2 -translate-y-1/2 right-3'>
                <img src={!stateGlobals.darkMode ? iconArrowDown : iconArrowDownDark} alt="Icon Arrow Down" className='w-2 max-md:w-1.75' />
              </figure>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-3 relative">
          ...
          />
          <figure className='absolute top-1/2 -translate-y-1/2 right-4 cursor-pointer'>
            <img src={!stateGlobals.darkMode ? iconSearch : iconSearchDark} alt="Icon Search" className='w-3.75 max-md:w-3' />
          </figure>
        </div>
      </div>
          ...
          {Array.from({ length: Math.min(table.getPageCount(), 5) }, (_, i) => {
            ...
            return (
              <button
                ...
                className={`w-7.5 max-md:w-7 h-7.5 max-md:h-7 flex justify-center items-center rounded-lg font-medium transition-all dark:border dark:border-background-white-15 cursor-pointer text-sm max-md:text-xs duration-300 ${pageIndex === pageNum ? 'bg-background-black-90 text-background-light dark:bg-background-white-8 shadow-md' : 'text-color-black-50 dark:text-color-white-80'}`}
              >
                {pageNum + 1}
              </button>
            );
