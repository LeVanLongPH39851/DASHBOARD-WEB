                    ...
                    >
                      {row.getVisibleCells().map((cell, cellIdx) => {
                        ...
                        return (
                          ...
                          >
                            {rawValue && customCol[columnName]?.channel ? <figure><img src={`${window.location.origin}${path}channels/${CHANNELS[rawValue]}`} alt={rawValue} className='w-15 max-lg:w-13 max-md:w-11' /></figure> : rawValue && customCol[columnName]?.flag2 ? <div className='items-center gap-1.5 inline-flex text-nowrap'><figure><img src={`${window.location.origin}${path}flags/${FLAGS[rawValue.split(' - ')[0]]}`} alt={rawValue.split(' - ')[0]} className='w-7 max-lg:w-6 max-md:w-5' /></figure>{RESULT[rawValue] || rawValue}<figure><img src={`${window.location.origin}${path}flags/${FLAGS[rawValue.split(' - ')[1]]}`} alt={rawValue.split(' - ')[1]} className='w-7 max-lg:w-6 max-md:w-5' /></figure></div> : rawValue && customCol[columnName]?.flag ? <div className='items-center gap-2 inline-flex'><figure><img src={`${window.location.origin}${path}flags/${FLAGS[rawValue]}`} alt={rawValue} className='w-7 max-lg:w-6 max-md:w-5' /></figure>
