...
const ChildTabs = ({ tabs, defaultTab, variant = "default" }) => {
  ...
  const variants = {
    default: {
      container:
        "relative flex gap-1 mb-6 max-lg:mb-5 max-md:mb-4 overflow-hidden rounded-2xl p-1 bg-background-black-4 dark:bg-background-white-8 w-fit transition-all duration-500",
      button: (isActive) =>
        `relative px-6 max-lg:px-5 max-md:px-4 py-2 font-normal text-sm max-lg:text-[13px] max-md:text-xs transition-all duration-300 cursor-pointer z-10 ${
          isActive
      ...
      background:
        "absolute inset-0 bg-background-black-child-tab dark:bg-color-white-80 rounded-2xl transition-all duration-500 ease-out",
    },
    pills: {
...
export default React.memo(ChildTabs);
