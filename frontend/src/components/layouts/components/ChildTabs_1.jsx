...
export default function ChildTabs({ tabs, defaultTab, variant = "default" }) {
  ...
  const variants = {
    default: {
      container: "flex mb-2",
      button: (isActive) =>
        `px-6 py-1 font-medium text-md transition-colors cursor-pointer ${
          isActive
            ? "border-b-2 border-color-light text-white bg-background-dark rounded-4xl"
    ...
    pills: {
      ...
      button: (isActive) =>
        `px-6 py-1 rounded-full font-medium text-md transition-all ${
          isActive
    ...
    bg: {
      ...
      button: (isActive) =>
        `flex-1 px-6 py-2 rounded-md font-medium text-sm transition-all ${
          isActive
