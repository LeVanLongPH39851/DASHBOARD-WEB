 ...
 */
const NormalTabs = ({ tabs, defaultTab, variant = "default" }) => {
  // console.log("NormalTabs");
  ...
  const variants = {
    default: {
      container:
        "relative flex gap-1 mb-6 max-lg:mb-5 max-md:mb-4 overflow-hidden rounded-2xl p-1 bg-background-black-4 dark:bg-background-white-8 w-fit transition-all duration-500", // ✅ Không bg-gray-100
      button: (isActive) =>
        `relative px-6 max-lg:px-5 max-md:px-4 w-36 max-md:w-30 py-2 font-normal text-sm max-lg:text-[13px] max-md:text-xs transition-all duration-300 cursor-pointer z-10 ${
          isActive
      ...
      background:
        "absolute inset-0 bg-background-black-child-tab dark:bg-color-white-80 rounded-2xl transition-all duration-500 ease-out", // ✅ background-dark
    },
    pills: {
      container: "flex gap-2 mb-2",
      button: (isActive) =>
        `px-6 py-1 rounded-full font-medium text-md transition-all duration-300 z-10 ${
          isActive
    ...
    bg: {
      ...
      button: (isActive) =>
        `relative flex-1 px-6 py-2 rounded-md font-medium text-sm transition-all duration-300 z-10 ${
          isActive
            ? "bg-white text-blue-600 shadow-sm font-semibold" // ✅ Giữ nguyên blue-600 như ChildTabs
            : "text-gray-600 hover:text-gray-900"
        }`,
      background:
        "absolute inset-0 bg-white rounded-md transition-all duration-500 ease-out shadow-inner",
    },
  };
  ...
  return (
    <div>
      ...
      >
        ...
        {showBgOverlay && (
          <div className={currentVariant.background} style={bgStyle} />
        )}
      </div>
...
export default React.memo(NormalTabs);
