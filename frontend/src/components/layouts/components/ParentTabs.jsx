...
const ParentTabs = ({
  ...
  const variants = {
    default: {
      ...
      button: (isActive) =>
        `py-2 text-nowrap uppercase max-md:normal-case font-medium text-[16px] max-lg:text-sm max-md:text-xs transition-colors duration-300 cursor-pointer flex gap-2 max-lg:gap-1.5 max-md:flex-col max-md:justify-center items-center ${
          isActive
            ? "text-color-primary-700 font-semibold"
            : "text-color-black-50 dark:text-color-white-50 hover:text-gray-700 hover:text-gray-700 dark:hover:text-color-white-80"
        }`,
      underline:
        "absolute -bottom-px max-md:-top-px block h-[3px] bg-color-primary-700 transition-all duration-400 ease-out rounded-full",
    },
    // ... giữ nguyên pills, bg
    pills: {
      ...
      button: (isActive) =>
        `px-8 py-3 rounded-full font-semibold text-lg transition-all duration-300 ${
          isActive
            ? "bg-color-primary-700 text-white shadow-lg"
    ...
    bg: {
      ...
      button: (isActive) =>
        `flex-1 px-8 py-3 rounded-lg font-semibold text-lg transition-all duration-300 ${
          isActive
            ? "bg-white text-color-primary-700 shadow-md"
  ...
  const stickyClasses = sticky
    ? `sticky max-md:fixed max-md:left-0 max-md:bottom-0 max-md:top-auto ${zIndex} bg-background-light transition-all duration-300 dark:bg-background-dark shadow-xs`
    : "";
      ...
      >
        {tabs.map((tab, index) => (
          ...
          >
            <figure>
              <img
                src={activeTab !== tab.id ? tab.icon : tab.iconActive}
                className="w-5"
                alt="Icon"
              />
            </figure>
        ...
        {showUnderline && (
          <span className={currentVariant.underline} style={underlineStyle} />
        )}
      </div>
...
export default React.memo(ParentTabs);
