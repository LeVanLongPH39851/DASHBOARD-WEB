...
const Filter = ({ filters, horizontalFixed = false }) => {
  ...
  const ALL_REGIONALS = useMemo(
    () => [
      ...
      {
        value: "Nam Trung Bộ và Tây Nguyên",
        label: "Nam Trung Bộ và Tây Nguyên",
      },
      {
  ...
  const ALL_TIMEBANDS = useMemo(
    () => [
      { value: "00h - 01h", label: "00h - 01h" },
      { value: "01h - 02h", label: "01h - 02h" },
      { value: "02h - 03h", label: "02h - 03h" },
      { value: "03h - 04h", label: "03h - 04h" },
      { value: "04h - 05h", label: "04h - 05h" },
      { value: "05h - 06h", label: "05h - 06h" },
      { value: "06h - 07h", label: "06h - 07h" },
      { value: "07h - 08h", label: "07h - 08h" },
      { value: "08h - 09h", label: "08h - 09h" },
      { value: "09h - 10h", label: "09h - 10h" },
  ...
  const DEFAULT_RANGE_HOUR = useMemo(() => ({ min: 0, max: 1439 }), []);
  ...
  useEffect(() => {
    ...
    const lookups = {
      ...
      events: ALL_EVENTS,
      provinces: ALL_PROVINCES[0].value
        ? ALL_PROVINCES
        : sessionStorage.getItem(FILTERS) !== "{}"
          ? JSON.parse(sessionStorage.getItem(FILTERS)).provinces
          : ALL_PROVINCES,
      regionals: ALL_REGIONALS,
      keyCities: ALL_KEY_CITIES,
      timebands: ALL_TIMEBANDS,
      firstLevels: ALL_FIRST_LEVELS,
      programs: ALL_PROGRAMS[0].value
        ? ALL_PROGRAMS
        : sessionStorage.getItem(FILTERS) !== "{}"
          ? JSON.parse(sessionStorage.getItem(FILTERS)).programs
          : ALL_PROGRAMS,
    };
      ...
      >
        ...
        >
          ...
          >
            ...
            {currentTab == DISABLE_TABS.channel && (
              <SelectMultiFilter
                label="Khung giờ"
            ...
            {currentTab != DISABLE_TABS.rating_by_minute &&
              (!horizontal ? (
                <CheckboxMultiFilter
                  label="Live/TSV"
                  placeholder="Chọn event..."
                  options={ALL_EVENTS}
                  value={filterValues?.events || VALUE_DEFAULT}
                  onChange={createFilterChangeHandler("events")}
                  marginBottom="mb-4 max-lg:mb-3 max-md:mb-2"
                />
              ) : (
                <SelectMultiFilter
            ...
            {currentTab == DISABLE_TABS.program && (
              <RangeFilter
                label="Giờ phát sóng"
                min={0}
                max={1439}
                step={1}
                value={filterValues?.startHours || DEFAULT_RANGE_HOUR}
                onChange={createFilterChangeHandler("startHours")}
                marginBottom={"mb-4 max-lg:mb-3 max-md:mb-2"}
                horizontalFixed={horizontalFixed}
              />
            )}
            ...
            {[DISABLE_TABS.program, DISABLE_TABS.rating_by_minute].includes(
              currentTab,
            ) && (
              <SelectMultiFilter
                label="Chương trình"
