...
const FilterSpot = ({ filters, horizontalFixed = false }) => {
  ...
  const ALL_KEY_CITIES = useMemo(
    () => [{ value: "4 TP lớn", label: "4 TP lớn" }],
    [],
  );

  const ALL_TIMEBANDS = useMemo(
    () => [
  ...
  const ALL_FIRST_LEVELS = useMemo(
    () => [
  ...
  const ALL_PRODUCTS = useMemo(
    () => toOptions(filters?.filterProduct, "product"),
    [filters?.filterProduct],
  );
  const ALL_GROUPS = useMemo(
    () => toOptions(filters?.filterGroup, "group"),
    [filters?.filterGroup],
  );
  const ALL_CAMPAIGNS = useMemo(
    () => toOptions(filters?.filterCampaign, "campaign_name"),
    [filters?.filterCampaign],
  );
  const ALL_BRANDS = useMemo(
    () => toOptions(filters?.filterBrand, "brand"),
    [filters?.filterBrand],
  );
  const ALL_ADVERTISERS = useMemo(
    () => toOptions(filters?.filterAdvertiser, "advertiser"),
    [filters?.filterAdvertiser],
  );
  const ALL_ADCODES = useMemo(
    () => toOptions(filters?.filterAdcode, "ads_code"),
    [filters?.filterAdcode],
  );
  ...
  const COMPONENTS = useMemo(
    () => [
      ...
      <CheckboxMultiFilter
        ...
        value={filterValues?.keyCities || VALUE_DEFAULT}
        onChange={createFilterChangeHandler("keyCities")}
        marginBottom="mb-4 max-lg:mb-3 max-md:mb-2"
        isSearch={false}
      />,
      <SelectMultiFilter
    ...
    [
      filterValues?.provinces?.length,
      filterValues?.regionals?.length,
      filterValues?.keyCities?.length,
      ALL_PROVINCES,
    ],
  );
          ...
          >
...

            {
              <SelectMultiFilter
                ...
                value={filterValues?.timebands || VALUE_DEFAULT}
                onChange={createFilterChangeHandler("timebands")}
                marginBottom="mb-4 max-lg:mb-3 max-md:mb-2"
                horizontalFixed={horizontalFixed}
              />
            }

            {currentTab !== DISABLE_TABS.effective && (
              <SelectMultiFilter
                label="Adcode"
                placeholder="Chọn Adcode"
                options={ALL_ADCODES}
                value={filterValues?.adCodes || VALUE_DEFAULT}
                onChange={createFilterChangeHandler("adCodes")}
                marginBottom="mb-4 max-lg:mb-3 max-md:mb-2"
                horizontalFixed={horizontalFixed}
              />
            )}

            {currentTab !== DISABLE_TABS.effective && !horizontal && (
              <CheckboxMultiFilter
                label="Loại spot"
                placeholder="Chọn loại spot"
                options={ALL_SPOT_TYPES}
                value={filterValues?.spotTypes || VALUE_DEFAULT}
                onChange={createFilterChangeHandler("spotTypes")}
                marginBottom="mb-4 max-lg:mb-3 max-md:mb-2"
              />
            )}
            ...
            <SelectMultiFilter
              ...
              options={ALL_GROUPS}
              value={filterValues?.groups || VALUE_DEFAULT}
              onChange={createFilterChangeHandler("groups")}
              marginBottom="mb-4 max-lg:mb-3 max-md:mb-2"
              horizontalFixed={horizontalFixed}
            />
            ...
            <SelectMultiFilter
              ...
              value={filterValues?.advertisers || VALUE_DEFAULT}
              onChange={createFilterChangeHandler("advertisers")}
              marginBottom="mb-4 max-lg:mb-3 max-md:mb-2"
              horizontalFixed={horizontalFixed}
            />
            ...
            <SelectMultiFilter
              ...
              value={filterValues?.products || VALUE_DEFAULT}
              onChange={createFilterChangeHandler("products")}
              marginBottom="mb-4 max-lg:mb-3 max-md:mb-2"
              horizontalFixed={horizontalFixed}
            />
            ...
            <SelectMultiFilter
              ...
              options={ALL_BRANDS}
              value={filterValues?.brands || VALUE_DEFAULT}
              onChange={createFilterChangeHandler("brands")}
              marginBottom="mb-4 max-lg:mb-3 max-md:mb-2"
              horizontalFixed={horizontalFixed}
            />
            ...
            <SelectMultiFilter
              ...
              value={filterValues?.campaigns || VALUE_DEFAULT}
              onChange={createFilterChangeHandler("campaigns")}
              marginBottom="mb-4 max-lg:mb-3 max-md:mb-2"
              horizontalFixed={horizontalFixed}
            />

            {[
              DISABLE_TABS.effective,
              DISABLE_TABS.ad_monitoring_report,
            ].includes(currentTab) &&
              !horizontal && (
                <CheckboxMultiFilter
                  label="Thiết bị"
                  ...
                  value={filterValues?.platforms || VALUE_DEFAULT}
                  onChange={createFilterChangeHandler("platforms")}
                  marginBottom="mb-4 max-lg:mb-3 max-md:mb-2"
                />
              )}
            ...
            {!horizontal ? (
              <CheckboxMultiFilter
                ...
                value={filterValues?.firstLevels || VALUE_DEFAULT}
                onChange={createFilterChangeHandler("firstLevels")}
                marginBottom="mb-4 max-lg:mb-3 max-md:mb-2"
              />
            ) : (
              <SelectMultiFilter
            ...
            {!horizontal && currentTab === DISABLE_TABS.effective && (
