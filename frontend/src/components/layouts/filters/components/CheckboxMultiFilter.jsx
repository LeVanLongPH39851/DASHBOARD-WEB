...
const CheckboxMultiFilter = ({
  ...
  // console.log("CheckboxMultiFilter " + label);
  ...
  return (
    <div className={isOpenFilter ? marginBottom : ""}>
      ...
      <div
        className={`transition-all duration-300 relative filter-relative overflow-hidden`}
        data-initial-height="match"
      >
        <div
          className={`transition-all duration-300 absolute w-full left-0 filter-absolute ${isOpenFilter ? "visible opacity-100 top-0" : "invisible opacity-0 -top-1/2"}`}
        >
          {/* Search */}
          {isSearch && (
            <div className="flex items-center gap-2 px-4 max-lg:px-3 max-md:px-3 py-2.5 max-lg:py-1.5 max-md:py-1.5 mb-1.5 border border-background-line-gray rounded-xl bg-background-light dark:border-background-white-15 transition-all duration-300 dark:bg-background-white-8 max-h-10">
              <img
                src={!darkMode ? iconSearch : iconSearchDark}
                className="w-3.5 h-3.5 max-lg:w-3.25 max-lg:h-3.25 max-md:w-3 max-md:h-3"
                alt="Icon Search"
              />
              <input
                type="text"
                ...
                className="w-full text-sm max-lg:text-[13px] max-md:text-xs outline-none text-color-black-50 font-normal placeholder:text-color-black-50 dark:text-color-white-50 dark:placeholder:text-color-white-50 bg-transparent transition-all duration-300"
              />
              {search && (
                <button
                  type="button"
            ...
            {filteredOptions.length > 0 ? (
              filteredOptions.map((opt) => (
                <li
                  ...
                  className={`
                        flex items-center gap-2 max-lg:gap-1.5 px-3 max-lg:px-2.5 max-md:px-2 py-1.25 max-md:py-1 rounded-xl cursor-pointer text-sm max-lg:text-[13px] max-md:text-xs font-normal transition-all duration-300
                        ${
                          isChecked(opt.value)
                ...
                >
                  <div
                    className={`
                        w-4 max-md:w-3 max-lg:w-3.5 h-4 max-lg:h-3.5 max-md:h-3 shrink-0 rounded-sm border-2 transition-all duration-300
                  ...
                  >
                    {isChecked(opt.value) && (
                      <svg
                        viewBox="0 0 10 8"
                        className="w-2.5 max-md:w-2 h-2.5 max-md:h-2"
                        fill="none"
                      >
                        <path
                          d="M1 4L3.5 6.5L9 1"
              ...
              <li className="px-3 max-lg:px-2.5 max-md:px-2 py-2 max-md:py-1 text-sm max-md:text-xs text-[rgba(0,0,0,0.4)] dark:text-color-white-90 transition-all duration-300">
                Không có dữ liệu
              </li>
            )}
          </ul>
...
export default React.memo(CheckboxMultiFilter);
