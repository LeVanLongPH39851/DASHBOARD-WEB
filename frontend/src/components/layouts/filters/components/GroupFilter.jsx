...
const GroupFilter = ({ title, components = [] }) => {
  ...
  const openFilter = sessionStorage.getItem("open_filters")
    ? JSON.parse(sessionStorage.getItem("open_filters")).includes(title)
    : false;
  ...
  return (
    <div>
      ...
      >
        <label className="cursor-pointer text-[16px] max-lg:text-sm max-md:text-xs text-color-p-login dark:text-color-white-50 transition-all duration-300 font-medium">
          {title}
        </label>
        <figure
      ...
      <div
        className={`transition-all duration-100 relative filter-relative overflow-hidden`}
