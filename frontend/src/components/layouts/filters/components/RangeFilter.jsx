...
const RangeFilter = ({
  ...
  const handleMinInputText = (newMin) => {
    ...
    if (
      newMin.length > 5 ||
      (newMin[0] && !/^[0-2]/.test(newMin[0])) ||
      (newMin[1] &&
        (newMin[0] < 2 ? !/\d/.test(newMin[1]) : !/^[0-4]/.test(newMin[1]))) ||
      (newMin[2] && newMin[2] !== ":") ||
      (newMin[3] && !/^[0-5]/.test(newMin[3])) ||
      (newMin[4] && !/\d/.test(newMin[4]))
    )
      return;
  ...
  const handleMaxInputText = (newMax) => {
    ...
    if (
      newMax.length > 5 ||
      (newMax[0] && !/^[0-2]/.test(newMax[0])) ||
      (newMax[1] &&
        (newMax[0] < 2 ? !/\d/.test(newMax[1]) : !/^[0-4]/.test(newMax[1]))) ||
      (newMax[2] && newMax[2] !== ":") ||
      (newMax[3] && !/^[0-5]/.test(newMax[3])) ||
      (newMax[4] && !/\d/.test(newMax[4]))
    )
      return;
  ...
  const nextPrev = (state, setState, setStateText, next, start) => {
    ...
    if (con) {
      ...
      onChange?.({
        min: start ? newValue : safeMin,
        max: start ? safeMax : newValue,
      });
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {!horizontalFixed && (
            <div className="flex items-center gap-2 max-lg:gap-1.5 mb-1">
              <div className="w-1/2 relative">
                ...
                <figure
                  onClick={() =>
                    nextPrev(localMin, setLocalMin, setLocalMinText, true, true)
                  }
                  className={`cursor-pointer transition-all duration-300 absolute top-2.5 max-lg:top-2.25 max-md:top-2 px-1 right-2`}
                >
                  <img
                    src={!darkMode ? iconArrowUpGray : iconArrowUpGrayDark}
                    className="w-2 max-lg:w-1.75 max-md:w-1.5"
                    alt="Icon Arrow Up Gray"
                  />
                </figure>
                <figure
                  ...
                  className={`cursor-pointer transition-all duration-300 absolute bottom-2.5 max-lg:bottom-2.25 px-1 max-md:bottom-2 right-2 rotate-180`}
                >
                  <img
                    src={!darkMode ? iconArrowUpGray : iconArrowUpGrayDark}
                    className="w-2 max-lg:w-1.75 max-md:w-1.5"
          ...
          <div
            className={`range-slider relative h-6 max-lg:h-5.5 max-md:h-5 flex items-center ${horizontalFixed ? "w-34 max-lg:w-32 max-md:w-full" : ""}`}
          >
            <div
              className={`absolute w-full h-1.25 max-lg:h-1 max-md:h-1 rounded-full bg-background-black-4 dark:bg-background-white-15`}
            />
            <div
