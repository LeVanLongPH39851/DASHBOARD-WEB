...
const InforFilter = ({ inforFilter, setInforFilter, filters, channels, events, provinces, setChannels, setEvents, setProvinces, currentTab, isOpen, setIsOpen }) => {
  ...
  return (
    <section className={`mx-6 px-6 py-4 bg-background-light border border-border-black-10 rounded-2xl sticky top-61 ${!isOpen.isInfor ? 'hidden' : ''}`} style={{zIndex: 100}}>
      ...
      <div className={`w-full transition-all duration-300 relative ${isOpen.horizontal && isOpen.isOpen ? 'min-h-[38.5px] mb-4' : 'min-h-0'}`}>
