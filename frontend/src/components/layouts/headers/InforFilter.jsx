    ...
    const hTab =
      window.innerWidth < 1025
        ...
        : (document.getElementById("Tab")?.getBoundingClientRect()?.height ??
          0);
    const hInforTab =
      document.getElementById("inforTabSticky")?.getBoundingClientRect()
        ?.height ?? 0;
    const h = hBreadCrumb + hTab + hInforTab;
    setHeight((prev) => (prev === h.toFixed(2) ? prev : h.toFixed(2)));
  }, [
    document.getElementById("Tab")?.getBoundingClientRect()?.height,
    document.getElementById("BreadCrumb")?.getBoundingClientRect()?.height,
    document.getElementById("inforTabSticky")?.getBoundingClientRect()?.height,
  ]);

  return (
    <section
      id="inforFilterSticky"
      className="mx-6 max-lg:mx-5 max-md:mx-4 bg-background-dashboard dark:bg-background-dashboard-dark transition-all duration-300 sticky rounded-b-2xl"
      style={{ zIndex: 100, top: `${height}px` }}
    >
      <div
        id="inforFilterRelative"
        className="transition-all duration-300 w-full"
      >
        <div
          id="inforFilter"
          className={`px-6 max-lg:px-5 max-md:px-4 py-4 max-lg:py-3 max-md:py-2.5 bg-background-light dark:bg-background-chart-dark w-full border transition-all duration-300 absolute left-0 border-border-black-10 dark:border-background-white-15 rounded-2xl ${isInfor ? "visible opacity-100 top-0" : "invisible opacity-0 -top-1/2"}`}
        >
          <div className="flex gap-2 mb-2.5 max-lg:mb-2 max-md:mb-1.5">
            <Button
              background={"bg-background-primary-2"}
              color={"text-background-check-box"}
              src={iconArrowLefLineDark}
              rotate={`${isOpen ? (!horizontal ? "" : "rotate-90") : !horizontal ? "rotate-180" : "rotate-270"} transition-all duration-300`}
              heightImage={"h-2.5 max-lg:h-2.25 max-md:h-2"}
              alt="Icon Arrow Down Line"
              text={`Bộ lọc (${Object.entries(appliedFilters || {}).filter(([, values]) => Array.isArray(values) && values.length > 0).length + 1 || 1})`}
              click={useCallback(() => {
                document
                  .getElementById("inforFilterRelative")
                  .classList.replace("duration-300", "duration-0");
                setIsOpen((prev) => !prev);
              }, [])}
            />
            <Button
              ...
              widthImage="w-3 max-lg:w-2.75 max-md:w-2.5"
              heightImage="h-3 max-lg:h-2.75 max-md:h-2.5"
              alt="Icon Arrow Nav Left"
              text={""}
              src2={!darkMode ? iconArrowDown : iconArrowDownDark}
              widthImage2="w-2 max-lg:w-1.75 max-md:w-1.5"
              alt2="Icon Arrow Down"
              rotate2={`${horizontal ? "-rotate-90" : ""} transition-all duration-300`}
              click={useCallback(() => {
                document
                  .getElementById("inforFilterRelative")
                  .classList.replace("duration-300", "duration-0");
          ...
          <div
            id="filterHorizontalRelative"
            className={`w-full overflow-hidden transition-all duration-300 relative ${horizontal && isOpen ? "mb-2.5 max-lg:mb-2 max-md:mb-1.5" : ""}`}
          >
            <div
              id="filterHorizontal"
              className={`w-full absolute left-0 transition-all duration-300 ${horizontal && isOpen ? "visible opacity-100 top-0" : "invisible opacity-0 -top-1/2"}`}
            >
              {horizontal && (
                <FilterComponent filters={filters} horizontalFixed={true} />
              )}
            </div>
          </div>
          <div className="flex gap-4 max-lg:gap-3 max-md:gap-2 items-center flex-wrap">
                ...
                valueFilters={[
                  formatDate(
                    appliedFilters?.startDate ||
                      (nameFilter == "FilterRating" ||
                      nameFilter == "FilterWorldCup"
                        ? yesterday
                        : beforeYesterday),
                  ),
                  formatDate(appliedFilters?.endDate || yesterday),
                ]}
                space={" - "}
              />
              {appliedFilters &&
            ...
            {Object.entries(appliedFilters || {}).filter(
              ...
              <span
                ...
                className="text-sm max-lg:text-[13px] max-md:text-xs font-normal text-color-black-50 dark:text-color-white-50 transition-all duration-300 underline cursor-pointer"
              >
                Xóa toàn bộ
              </span>
            )}
...
export default React.memo(InforFilter);
