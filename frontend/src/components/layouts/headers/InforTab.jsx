  ...
  return (
    <section id='inforTabSticky' className={`px-6 max-lg:px-5 max-md:px-4 py-3 flex gap-2.5 max-lg:gap-2 h-12 max-lg:h-11 max-md:h-10 items-center sticky top-34 max-lg:top-[113.93px] max-md:top-18 bg-background-dashboard dark:bg-background-dashboard-dark transition-all duration-300 border-b ${!stateGlobals.isInfor ? ' border-border-black-10 dark:border-background-white-15' : 'border-transparent'}`} style={{zIndex: 200}}>
        ...
        <div className='flex items-center gap-1 max-md:hidden'>
            ...
            <span className='text-color-black-100 dark:text-color-white-80 transition-all duration-300 font-medium text-sm max-lg:text-[13px]'>Dữ liệu ngày {formatDate(appliedFilters?.startDate || getYesterday())} - {formatDate(appliedFilters?.endDate || getYesterday())}</span>
        ...
        <span className='text-color-black-50 font-medium text-sm max-lg:text-[13px] dark:text-color-white-50 transition-all duration-300 max-md:text-xs text-nowrap max-md:hidden'>Ngày xử lý {formatDate(getYesterday())} 09:00</span>
