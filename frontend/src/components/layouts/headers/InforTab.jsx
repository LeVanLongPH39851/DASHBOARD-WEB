...
import { getYesterday, formatDate, formatDateTime } from '../../../helpers/helper';
...
const InforTab = ({ inforTab, maxInsert = false }) => {
  ...
  useEffect(() => {
    ...
    const h = Number(hBreadCrumb) + Number(hTab)
    setHeight(h.toFixed(2));
  }, [document.getElementById("Tab")?.getBoundingClientRect()?.height, document.getElementById("BreadCrumb")?.getBoundingClientRect()?.height]);

  return (
    <section id='inforTabSticky' className={`px-6 max-lg:px-5 max-md:px-4 py-3 flex gap-2.5 max-lg:gap-2 h-12 max-lg:h-11 max-md:h-10 items-center sticky bg-background-dashboard dark:bg-background-dashboard-dark transition-all duration-300 border-b ${!stateGlobals.isInfor ? ' border-border-black-10 dark:border-background-white-15' : 'border-transparent'}`} style={{ zIndex: 200, top: `${height}px` }}>
      <span className='text-color-black-100 dark:text-color-white-80 transition-all duration-300 font-normal text-sm max-lg:text-[13px] max-md:text-xs text-nowrap uppercase'>{inforTab}</span>
      ...
      <span className='text-color-black-50 font-normal text-sm max-lg:text-[13px] dark:text-color-white-50 transition-all duration-300 max-md:text-xs text-nowrap max-md:hidden'>Ngày xử lý {maxInsert ? formatDateTime(maxInsert) : formatDate(new Date()) + ' 09:00'}</span>
      ...
      <div className='flex-1 flex justify-end pr-1'><figure className='p-1 cursor-pointer' onClick={() => { document.getElementById('inforFilterRelative').classList.replace('duration-0', 'duration-300'); setStateGlobals(prev => ({ ...prev, isInfor: !prev.isInfor })); }}><img src={!stateGlobals.darkMode ? iconArrowDown : iconArrowDownDark} className={`w-2.25 max-md:w-2 ${stateGlobals.isInfor ? 'rotate-180' : ''} transition-all duration-300`} alt="Icon Arrow Down" /></figure></div>
